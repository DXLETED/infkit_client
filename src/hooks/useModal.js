...
export const useModal = ({initState = false, fixed = false, fullScreen = false, list = false, padding = true, width = 30, onopen, onclose, closeIf} = {}) => {
  const [vis, setVis] = useState(initState)
  const visRef = useRef(initState)
  const id = useMemo(() => generateId(), [])
  const [link, setLink] = useState()
  const dclick = e => (!closeIf || closeIf() === true) && visRef.current && e.target.closest('body') && e.target.tagName !== 'INPUT' && ((fixed || fullScreen)
