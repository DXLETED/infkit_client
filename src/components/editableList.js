...
export const EditableList = props => {
  return <div className={cn('editable-list-wr', props.className, {extended: props.extended, cpointer: props.cpointer, m: props.m, m2: props.m2})}>
    <MLabel d={props.label} r={props.limit && `${props.data.length} / ${props.limit}`} bg />
      ...
      {props.data.map((el, i) => <div className="editable-list-el-wr" key={i}>
        {isValidElement(el)
          ...
          : <>
              ...
              <div className="delete" onClick={() => !el.fixed && props.delete(i)}><img src={el.img || (!el.fixed ? '/static/img/delete.png' : '')} /></div>
            </>}
      </div>)}
