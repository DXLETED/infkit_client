    ...
    <div id="header">
      <header>
        <div className="left-list">
          <NavLink exact to="/" className="header-el main"><img src="/static/img/logo.png" />{location.pathname !== '/' && 'INFINITY KIT'}</NavLink>
          {cookie.guild
            ? <>
        ...
        <div className="right-list">
          {location.pathname !== '/' && guilds && cookie.guild && guilds.find(g => g.id === cookie.guild) && (() => {
            ...
            return <Dropdown className="header-el guild" dropdown={guilds.filter(g => g.bot).map((g, i) => <div onClick={() => {
                ...
                <div className="info">{g.name}<div className="id user-select" onClick={e => e.stopPropagation()}>{g.id}</div></div>
              </div>)}>
          ...
          {isAuthenticated && user
            ? <Dropdown className="header-el discord-user" dropdown={<div onClick={() => {
                dispatch({type: 'UPDATE_AUTH', data: {}})
                dispatch({type: 'UPDATE_USER', data: {}})
                dispatch({type: 'UPDATE_AUTHORIZED', data: false})
              }}><img src="/static/img/logout.png" />LogOut</div>}>
