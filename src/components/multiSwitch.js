    ...
    <div className={cn('multi-switch-wr', props.className, {v: vis, m: props.m})}>
      ...
      <div className="multi-switch" ref={ref}>
        ...
        {props.custom && (!props.type || props.type === 'string') && <div className={cn('option', 'custom', {selected: selected.custom})}>
          <Input onClick={() => props.set && props.set({...selected, custom: true, v: selected.v})} value={selected.custom && selected.v} set={n => props.set && props.set({...selected, custom: true, v: props.type === 'string' ? n : parseFloat(n)})} placeholder="CUSTOM" limit={props.limit} defsize fill center nb nm />
        </div>}
        ...
        {props.custom && props.type === 'percentage' && <div className={cn('option', 'custom-percentage', {selected: selected.custom})}>
          <Input onClick={() => props.set && props.set({...selected, custom: true, v: selected.v})} value={selected.custom && selected.v * 100} set={n => props.set && props.set({...selected, custom: true, v: n / 100})} placeholder="CUSTOM" fill center nb nm />
